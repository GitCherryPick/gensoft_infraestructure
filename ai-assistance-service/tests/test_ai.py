...
def test_create_user():
    ...
    assert response.json()["total_activities_answered"] == 0
    assert "last_accessed" in response.json()

def test_get_user():
    response = client.get("/student-stats/1")
    assert response.status_code == 200
    assert response.json()["student_id"] == stat_data["student_id"]
...
def test_update_user():
    ...
    response = client.put("/student-stats/1", json=update_data)
    print(response.json())
    assert response.status_code == 200
    assert response.json()["student_id"] == stat_data["student_id"]
...
def test_delete_user():
    response = client.delete("/student-stats/1")
    assert response.status_code == 200
